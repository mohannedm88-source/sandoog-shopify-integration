...
/**
 * TEMP diagnostic: fetch Sandoog's real city_code list so we can replace the
 * REPLACE_ME placeholders in src/mapping/cityCodes.sample.json. Needs no
 * secret from the caller — uses this service's own configured
 * SANDOOG_API_KEY. Remove this route once city codes are filled in.
 */
app.get('/setup/fetch-city-codes', async (_req, res) => {
      try {
              const data = await sandoog.getCityCodes();
